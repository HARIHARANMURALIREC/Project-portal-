...
export type AdminNavKey = 'overview' | 'uploads' | 'marks'
...
const navItems: {
  ...
  { key: 'overview', label: 'Dashboard', to: '/admin', icon: LayoutDashboard },
  { key: 'uploads', label: 'Uploads', to: '/admin/uploads', icon: FileStack },
  { key: 'marks', label: 'Marks', to: '/admin/marks', icon: ClipboardList },
]
    ...
    <>
      ...
      {!collapsed && (
        <p className="border-b border-slate-200 px-4 py-3 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400">
          Role: <span className="font-medium text-slate-700 dark:text-slate-300">Administrator</span>
        </p>
      )}
