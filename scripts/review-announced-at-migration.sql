-- Announcement date for common reviews (when coordinator announces)
-- Paste into Supabase SQL Editor and run (after 0028 if not yet applied)
...
-- Drop prior signatures (with and without scheduled_end_at)
drop function if exists public.coordinator_schedule_review_for_all(text, timestamptz, text);
drop function if exists public.coordinator_schedule_review_for_all(text, timestamptz, text, timestamptz);
drop function if exists public.coordinator_reschedule_review(uuid, timestamptz, text);
drop function if exists public.coordinator_reschedule_review(uuid, timestamptz, text, timestamptz);
...
declare
  ...
  v_count integer := 0;
  v_batch_id text;
  v_announced timestamptz := coalesce(p_announced_at, now());
begin
  ...
  if is_lead_teacher() or is_admin() then
    insert into team_reviews (
      team_id, review_title, scheduled_at, scheduled_end_at, announced_at,
      remarks, created_by, schedule_group_id
    )
    select t.id, trim(p_review_title), p_scheduled_at, p_scheduled_end_at, v_announced,
...
begin
  ...
  if is_lead_teacher() or is_admin() then
    ...
    group by tr.schedule_group_id, tr.review_title, tr.scheduled_at, tr.scheduled_end_at, tr.announced_at
...
begin
  ...
  if is_lead_teacher() or is_admin() then
    ...
    set scheduled_at = p_scheduled_at,
        ...
        announced_at = coalesce(p_announced_at, announced_at, now()),
    ...
    set scheduled_at = p_scheduled_at,
        scheduled_end_at = p_scheduled_end_at,
        announced_at = coalesce(p_announced_at, tr.announced_at, now()),
        remarks = coalesce(nullif(trim(coalesce(p_remarks, '')), ''), remarks),
        updated_at = now()
    from teams t
...
grant execute on function public.coordinator_schedule_review_for_all(text, timestamptz, text, timestamptz, timestamptz) to authenticated;
grant execute on function public.coordinator_list_review_schedules() to authenticated;
grant execute on function public.coordinator_reschedule_review(uuid, timestamptz, text, timestamptz, timestamptz) to authenticated;
