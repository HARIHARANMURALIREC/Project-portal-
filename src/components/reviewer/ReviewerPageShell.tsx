...
import { getSectionReviewerLabel, isSectionReviewer } from '@/lib/sectionReviewers'
import { teacherHomePath } from '@/lib/teacherRoutes'
import type { TeacherNavKey } from '@/types/teacher'

const NAV_ITEMS: { key: TeacherNavKey; label: string; to: string; icon: typeof LayoutDashboard }[] = [
  { key: 'dashboard', label: 'Dashboard', to: '/reviewer', icon: LayoutDashboard },
  { key: 'reviewer', label: 'Marks', to: '/reviewer/marks', icon: ClipboardCheck },
  { key: 'templates', label: 'Uploads', to: '/reviewer/uploads', icon: Upload },
  { key: 'sdg', label: 'SDG Page', to: '/reviewer/sdg', icon: Globe2 },
  { key: 'publications', label: 'Publications', to: '/reviewer/publications', icon: Newspaper },
  { key: 'profile', label: 'Profile', to: '/reviewer/profile', icon: User },
]

function UserAvatar({ name }: { name: string }) {
  const initial = name.trim().charAt(0).toUpperCase() || 'R'
  return (
    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-sky-100 text-sm font-semibold text-sky-700 ring-1 ring-sky-200 dark:bg-sky-950 dark:text-sky-300 dark:ring-sky-800">
      {initial}
    </div>
  )
}
    ...
    <>
      ...
      {!collapsed && (
        <p className="border-b border-slate-200 px-4 py-3 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400">
          Role: <span className="font-medium text-slate-700 dark:text-slate-300">Section Reviewer</span>
          {' · '}
          Sections: <span className="font-medium text-slate-700 dark:text-slate-300">{sectionLabel}</span>
        </p>
      )}
...
export function ReviewerPageShell({
  title,
  activeNav,
  children,
}: {
  ...
  if (!isSectionReviewer(profile)) {
    return <Navigate to={teacherHomePath(profile)} replace />
  }
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-app-black">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex min-w-0 flex-1 items-center justify-between gap-3">
              ...
              <div className="flex shrink-0 items-center gap-2">
                <ThemeToggle />
                <span className="rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700 ring-1 ring-sky-100 dark:bg-sky-950 dark:text-sky-300 dark:ring-sky-800 sm:px-3">
                  Reviewer · {sectionLabel}
                </span>
              </div>
            </div>
          </div>
        ...
        <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-white p-4 pb-4 dark:bg-app-black lg:p-6 lg:pb-6">
